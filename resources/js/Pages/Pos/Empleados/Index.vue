...
import type { PaginatedData, Empleado } from '@/types';

const props = defineProps<{
    empleados: PaginatedData<Empleado>;
    search?: string;
}>();
...
function handleDelete() {
    if (deleteId.value) {
        router.delete(route('pos.empleados.destroy', deleteId.value), {
            preserveScroll: true,
            onSuccess: () => {
...
<template>
    <AppPageShell>
        <AppPageHeader title="Empleados" description="Gestión de empleados">
            <template #actions>
                <Link
                    :href="route('pos.empleados.create')"
        ...
        <DataTable
            ...
            :current-page="empleados.current_page"
            :last-page="empleados.last_page"
            :from="empleados.from"
            :to="empleados.to"
            :search="search"
            base-route="pos.empleados.index"
            search-placeholder="Buscar por nombre, apellido o DNI..."
        >
            <template #cell-activo="{ row }">
                <BadgeActivo :activo="(row as unknown as Empleado).activo" />
            </template>
            <template #actions="{ row }">
                <div class="flex items-center gap-2">
                    <Link
                        :href="route('pos.empleados.edit', (row as unknown as Empleado).id)"
                    ...
                    <Button variant="ghost" size="sm" @click="confirmDelete((row as unknown as Empleado).id)">
                        Eliminar
                    </Button>
                </div>
            </template>
